...
def test_cwt_shape_mexh():
    ...
    dt = 0.01
    nf = 2
    wavelets = ['mexh', 'ricker']
    for wl in wavelets:
    	assert cwt(data=data, dt=dt, nf=nf, f_min=F_MIN, f_max=F_MAX, wl=wl, w0=None).shape == (600,2) 
...
def test_featurize_extend(tol=1e-7):
    ...
    assert_allclose(cwts, extended[:,1], atol=tol, rtol=tol)
